...
// Dashboard principal — tabla compacta con Polla, Puntos y Posición.
// Formato inspirado en la versión original de SuperGurú (2018).
...
export default async function DashboardPage() {
  ...
  const allPoolIds    = [...memberPoolIds]
  ...
  function getStanding(poolMemberId: string, poolId: string): { pos: number; total: number } {
    const myPts    = (myScores ?? []).find(s => s.pool_member_id === poolMemberId)?.total_points ?? 0
    const entries  = (allLeaderboard ?? []).filter(e => e.pool_id === poolId)
    const total    = entries.length
    const pos      = entries.filter(e => e.total_points > myPts).length + 1
    return { pos, total }
  }
  ...
  const hasPools = (memberRows?.length ?? 0) > 0 || adminOnlyRows.length > 0
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      {!hasPools ? (
        ...
        <div style={{
          ...
          {/* Cabecera de tabla — verde como en el original */}
          <div style={{
            display: 'grid',
          ...
          {/* Filas — miembro */}
          {(memberRows ?? []).map((entry, i) => {
            const pool = entry.pools as unknown as Pool
            if (!pool) return null
            const myPoints = (myScores ?? []).find(s => s.pool_member_id === entry.id)?.total_points ?? 0
            ...
            const isLast   = i === (memberRows?.length ?? 0) - 1 && adminOnlyRows.length === 0
          ...
          {/* Filas — admin sin ser miembro */}
          {adminOnlyRows.map((entry, i) => {
            const pool = entry.pools as unknown as Pool
            if (!pool) return null
            const isActive = new Date(pool.ends_at) > new Date()
...
// ---------------------------------------------------------------------------
function PoolRow({
  poolId, name, displayName, points, posLabel, pos,
  isActive, role, isLast, index,
}: {
  poolId: string
  name: string
  ...
  return (
    <Link href={`/pools/${poolId}`} style={{ textDecoration: 'none', display: 'block' }}>
      ...
      >
        {/* Nombre de la Polla + alias */}
        <div style={{ minWidth: 0 }}>
          <div style={{
            display: 'flex',
            alignItems: 'center',
            gap: '8px',
            marginBottom: displayName ? '3px' : 0,
            flexWrap: 'wrap',
          }}>
            <span style={{
              fontSize: '0.9375rem',
        ...
        <div style={{ textAlign: 'center' }}>
          <span style={{
            ...
            fontSize: '1.3rem',
            letterSpacing: '0.02em',
            color: role === 'admin' ? 'var(--color-text-muted)' : posColor,
            lineHeight: 1,
          }}>
            {posLabel}
          </span>
